...
# ── Full true latency-risk curves for decay rates B = 0.2 and B = 0.4 (lags 0–15)
...
gamma     <- 0.1
full_true <- bind_rows(
  data.frame(l = 0:15, B_label = "B==0.2",
             y = as.numeric(gamma * alpha_function(0:15, A = 0.3, B = 0.2))),
  data.frame(l = 0:15, B_label = "B==0.4",
             y = as.numeric(gamma * alpha_function(0:15, A = 0.3, B = 0.4)))
)
...
temp <- bind_rows(result_KnotGridSearch, result_TermSearch, result_Polynomial) %>%
  mutate(
    B_label  = paste0("B==", B),
    B_label  = factor(B_label,  levels = c("B==0.2", "B==0.4")),
...
full_true <- full_true %>%
  mutate(B_label = factor(B_label, levels = c("B==0.2", "B==0.4")))
