...
# True underlying curve: damped exponential weight A * exp(-B * l).
# B is the decay rate (varied across the study); A = 0.3 is fixed.

simulation_output <- function(sample_size, A = 0.3, B = 0.2,
  ...
  future_map_dfr(1:300, .f = function(sim_id){
    ...
    tryCatch({
      ...
      if (method == "Polynomial") {
        ...
        for (l in 1:latency) {
          log_HR_estimate[l] <- tryCatch(
            extract_CoxTermsearch(fit, lag = l - 1,
                                  latency = latency,
                                  knots   = 0:(latency - 1)),
            error = function(e) NA_real_)
          log_HR_true[l]     <- gamma * alpha_function(l - 1, A = A, B = B)
