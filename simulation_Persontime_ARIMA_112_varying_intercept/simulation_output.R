...
simulation_output <- function(sample_size, beta = 0.01, rho = 0.3, lambda = 0.5,  alpha1 = 0.3, alpha2 =-0.01, 
                              ...
                              lambda_0 = 0.5, gamma = 0.1, intercept = 0, degree = 4,
                              latency = 16,
  ...
  future_map_dfr(1:300, .f = function(sim_id){
    ...
    # fails (e.g. at very low event ratios a single replicate may not converge).
    log_HR_true <- vapply(1:latency, function(l) gamma * alpha_function(l - 1, knot = knot),
                          numeric(1))

    # `fit_failed` flags replicates where data generation or the model fit could
    # not produce an estimate. Such replicates get NA log_HR_estimate but are
    # still returned (with the true curve and event count where available) so the
    # batch is never interrupted -- the caller can simply count the failures.
    one_replicate <- function() {
      ...
      sim_data <- tryCatch(
        generate_data(sample_size, beta = 0.01, rho = 0.3, lambda = 0.5,
                      ...
                      lambda_0 = lambda_0, gamma = gamma, intercept = intercept),
      ...
      fit_res <- tryCatch({
        ...
        # A fit that silently returns NA estimates is also treated as a failure.
        list(est = est, failed = all(is.na(est)))
      }, error = function(e) {
        message(sprintf("sim_id %d failed (%s): imputing NA", sim_id, conditionMessage(e)))
        list(est = rep(NA_real_, latency), failed = TRUE)
      })

      data.frame(sim_id = sim_id,
                 l =  0:(latency -1),
                 log_HR_true = log_HR_true,
                 log_HR_estimate = fit_res$est,
                 n_events = n_events,
                 fit_failed = fit_res$failed)
    }

    # Outer guard: even an unexpected error outside the inner handlers (or a NULL
    # data set) yields a clean, fully-NA row block rather than aborting the batch.
    tryCatch(one_replicate(), error = function(e) {
      message(sprintf("sim_id %d hard-failed (%s): imputing NA", sim_id, conditionMessage(e)))
      data.frame(sim_id = sim_id,
                 l =  0:(latency -1),
                 log_HR_true = log_HR_true,
                 log_HR_estimate = NA_real_,
                 n_events = NA_real_,
                 fit_failed = TRUE)
    })
