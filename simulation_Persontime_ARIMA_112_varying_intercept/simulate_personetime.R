...
generate_data <- function(sample_size, beta = 0.01, rho = 0.3, lambda = 0.5, alpha1 = 0.3,  alpha2 =-0.01,
                          ...
                          lambda_0 = 0.1, gamma = 0.1, intercept = 0){
...
for (i in 1:max(df$person_id)){
  ...
  # `intercept` shifts the baseline log-hazard: a larger intercept gives a larger
  # lambda and hence more events, with the latency-risk curve (gamma) held fixed.
