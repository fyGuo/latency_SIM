...
# simulate for polynomials with sample_size = 5000, knot = 10, varying intercept and degree
temp <- data.frame()

for (intercept in c(-4, -2.5, -1.5, 0)){
  for (degree in 5) {
    result <- simulation_output(sample_size = 5000, knot = 10, intercept = intercept, degree = degree, method = "Polynomial")
