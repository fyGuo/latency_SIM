...
result_knot <- result_knot %>% group_by(positive, l) %>%
...
result_term <- result_term %>% group_by(positive, l) %>%
  summarise(log_HR_true = mean(log_HR_true),
            log_HR_estimate_mean = mean(log_HR_estimate),
            log_HR_estimate_se = sd(log_HR_estimate))
...
temp$positive_label <- ifelse(temp$positive, "Positive~constant~weights", "Zero~weights~(null~effect)")
temp$positive_label <- factor(temp$positive_label,
                 levels = c("Zero~weights~(null~effect)", "Positive~constant~weights"))
...
whole_plot <- ggplot(temp) +
  ...
  facet_grid(.~positive_label, labeller = label_parsed) +
...
temp_all$positive_label <- ifelse(temp_all$positive, "Positive~constant~weights", "Zero~weights~(null~effect)")
temp_all$positive_label <- factor(temp_all$positive_label,
                 levels = c("Zero~weights~(null~effect)", "Positive~constant~weights"))
...
ggplot(temp_all) +
  ...
  facet_grid(method~positive_label, labeller = label_parsed) +
