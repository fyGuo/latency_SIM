...
# Constant weighting: all lags receive the same weight A.
# positive = TRUE  -> weight = A for all lags (constant positive effect)
# positive = FALSE -> weight = 0 for all lags (null effect, no association)
alpha_function <- function(l, A = 0.3, positive = TRUE){
  if (positive) rep(A, length(l)) else rep(0, length(l))
}

# This function is to generate the simualton data
generate_data <- function(sample_size, beta = 0.01, rho = 0.3, lambda = 0.5, A = 0.3, positive = TRUE,
...
for (i in 1:max(df$person_id)){
  ...
  df_person$cumExposure <- as.matrix(df_person[,paste0("lag", 0:15)]) %*% alpha_function(0:15, A = A, positive = positive)
