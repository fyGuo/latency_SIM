...
# Unified weighting function supporting four curve shapes:
#   "unimodal" : B-spline with internal knot at `knot` (default 10)
#   "bimodal"  : mixture of two Gaussians at lags 3 and 12; B controls second mode
#                B = -1 -> second mode negative; B = 1 -> both modes positive
#   "decayed"  : exponential decay A * exp(-B * l); B controls decay rate
alpha_function <- function(l, curve = "unimodal",
                           A = 0.3, B = 0.4, knot = 10,
  ...
  } else if (curve == "bimodal") {
    A * (exp(-0.5 * ((l - 3) / 2)^2) + B * exp(-0.5 * ((l - 12) / 2)^2))
  } else if (curve == "decayed") {
    A * exp(-B * l)
  }
}
...
generate_data <- function(sample_size, beta = 0.01, rho = 0.3, lambda = 0.5,
  ...
  for (i in 1:max(df$person_id)) {
    ...
    df_person$cumExposure <- as.matrix(df_person[, paste0("lag", 0:15)]) %*%
      alpha_function(0:15, curve = curve, A = A, B = B, knot = knot,
