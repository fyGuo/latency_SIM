...
# Permutation test of H0 (weights constant across lags) under the exponentially decayed curve.
...
set.seed(400 + part)   # distinct seed per part (and per scenario)
result <- permutation_study(WEIGHT_CURVES$decayed, "decayed",
                            sample_size = 5000,
                            gamma       = 0.1,
                            lambda_0    = 0.5,
                            intercept   = 0,
...
saveRDS(result, sprintf("permtest_decayed_part%d.rds", part))

cat(sprintf("decayed part %d / %d: rejection rate = %.3f  (mean events = %.1f)\n",
            part, n_parts,
