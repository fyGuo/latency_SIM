...
# Permutation test of H0 (weights constant across lags) under the constant ZERO curve
# (c = 0: no exposure effect at any lag -- a true global null).
...
set.seed(500 + part)   # distinct seed per part (and per scenario)
result <- permutation_study(WEIGHT_CURVES$constant_low, "constant_low",
                            sample_size = 5000,
                            gamma       = 0.1,
                            lambda_0    = 0.5,
                            intercept   = 0,
...
saveRDS(result, sprintf("permtest_constant_low_part%d.rds", part))

cat(sprintf("constant_low part %d / %d: rejection rate = %.3f  (mean events = %.1f)\n",
            part, n_parts,
