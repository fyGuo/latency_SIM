#!/bin/bash
# Permutation test (bimodal_pos curve): 300 replicates split into 6 array jobs of
# 50 sims each (n_perm = 300); each job writes its own
# permtest_bimodal_pos_part<k>.rds, so a crashed job only loses its part.
# Previous monolithic runs hit the time limit at ~110/300 sims (~16 min/sim);
# 50 sims ~ 14 h, so 2 days leaves ample headroom.
#
# Usage:
#   sbatch run_permtest_bimodal_pos.sh              # all 6 parts
#   sbatch --array=3 run_permtest_bimodal_pos.sh    # resubmit part 3 only
#   sbatch --array=2,4-6 run_permtest_bimodal_pos.sh
#
...
R CMD BATCH --quiet --no-restore --no-save \
  "$DIR/permtest_bimodal_pos.R" \
  "$DIR/permtest_bimodal_pos_part${SLURM_ARRAY_TASK_ID}.out"
