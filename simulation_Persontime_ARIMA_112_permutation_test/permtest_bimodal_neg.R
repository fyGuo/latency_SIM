...
# Permutation test of H0 (weights constant across lags) under the bimodal (second bump negative) curve.
...
set.seed(300 + part)   # distinct seed per part (and per scenario)
result <- permutation_study(WEIGHT_CURVES$bimodal_neg, "bimodal_neg",
                            sample_size = 5000,
                            gamma       = 0.1,
                            lambda_0    = 0.5,
                            intercept   = 0,
...
saveRDS(result, sprintf("permtest_bimodal_neg_part%d.rds", part))

cat(sprintf("bimodal_neg part %d / %d: rejection rate = %.3f  (mean events = %.1f)\n",
            part, n_parts,
