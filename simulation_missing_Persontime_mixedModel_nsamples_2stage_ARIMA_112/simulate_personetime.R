...
generate_data <- function(sample_size, beta = 0.01, rho = 0.3, lambda = 0.5, alpha1 = 0.3,  alpha2 =-0.01, 
  ...
  entry_age <- sample(c(50), size = sample_size, replace = TRUE)
  ...
  for (i in 1:max(df$person_id)){
    ...
    df_person$cumExposure <- as.matrix(df_person[,paste0("lag", 0:15)]) %*% alpha_function(0:15, alpha1 = alpha1, alpha2 = alpha2, alpha3 = alpha3, alpha4 = alpha4, knot = knot)
    df_person <- df_person[seq(1, dim(df_person)[1], 2),]
    
    df_person$lambda <- lambda_0*exp(gamma * df_person$cumExposure + 0.1*(df_person$age-30))

    df_person$failure_time <- rexp(dim(df_person)[1], rate = df_person$lambda)
    df_person$failure <- if_else(df_person$failure_time > 2, 0, 1)
  ...
  tmp$age_end <- if_else(tmp$failure == 0, tmp$age_start + 1.999999, tmp$age_start + tmp$failure_time)
  
  
  # In this step, we introduced missingness in the exposures.
  # for each person, we can only observe lag0, lag1, lag2, lag3 in the first row,
  # and all the other lags are missing. Then in the follow-ups we can have more data
 ...
 # for the use of this analysis we just keep lag0 - lag19
 tmp <- tmp %>% dplyr::select(-paste0("lag", 20:30))
  return(tmp)
}
