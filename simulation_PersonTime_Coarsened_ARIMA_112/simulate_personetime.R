...
generate_data <- function(sample_size, beta = 0.01, rho = 0.3, lambda = 0.5, alpha1 = 0.3,  alpha2 =-0.01, 
  ...
  for (i in 1:max(df$person_id)){
    ...
    df_person <- mutate(df_person, year = row_number())
    ...
    df_person <- df_person[seq(1, dim(df_person)[1], 4),]
    ...
    df_person$failure <- if_else(df_person$failure_time > 4, 0, 1)
  ...
  tmp$age_end <- if_else(tmp$failure == 0, tmp$age_start + 3.999999, tmp$age_start + tmp$failure_time)
  tmp <- tmp %>%
    mutate(lag1 = lag4,
           lag2 = lag4,
           lag3 = lag4,
