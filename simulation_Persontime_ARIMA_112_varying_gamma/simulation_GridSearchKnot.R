...
# simulate for KnotGridSearch with sample_size = 5000, knot = 10, varying gamma
temp <- data.frame()

for (gamma in c(0.01, 0.05, 0.1)){
    result <- simulation_output(sample_size = 5000, knot = 10, gamma = gamma, method  = "GridSearchKnot")
    result$gamma <- gamma
