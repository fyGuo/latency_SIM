...
# simulate for polynomials with sample_size = 5000, knot = 10, varying gamma and degree
temp <- data.frame()

for (gamma in c(0.01, 0.05, 0.1)){
  for (degree in 5) {
    result <- simulation_output(sample_size = 5000, knot = 10, gamma = gamma, degree = degree, method = "Polynomial")
    result$gamma <- gamma
