...
# Smooth bimodal weighting: mixture of two Gaussians, modes at lag 5 and lag 10, sd = 2
# B controls the relative height of the second mode (lag 10) vs the first (lag 5):
#   B = 0.4 -> second peak is 40% of first (moderately asymmetric bimodal)
#   B = 0.2 -> second peak is 20% of first (primary peak at lag 5 dominant)
#   B = 0.1 -> second peak is 10% of first (weakly bimodal)
alpha_function <- function(l, A = 0.3, B = 0.2){
  w <- exp(-0.5 * ((l - 3) / 2)^2) + B * exp(-0.5 * ((l - 12) / 2)^2)
  A * w
}
