...
inner_imputation <- function(data) {
  ...
  tmp3 <- tmp %>% dplyr::mutate(age_start = age_start+2, lag0 = NA, origin = 0)
  tmp4 <- tmp %>% dplyr::mutate(age_start = age_start+3, lag0 = NA, origin = 0)
  newdata <- base::rbind(tmp1, tmp2, tmp3, tmp4) %>% dplyr::arrange(id, age_start)
